...
local function msOOQuery(sqlText, callback, errorCallback, queryValue)
    ...
    queryObject.onSuccess = function()
        local res = queryValue and data and data[1] and ({next(data[1],nil)})[2] or not queryValue and data or nil
...
local function tmsqlQuery(sqlText, callback, errorCallback, queryValue)
    local call = function(res)
        ...
        if queryValue and callback then return callback(res.data and res.data[1] and ({next(res.data[1],nil)})[2] or nil) end
